...
public class QuoteCursorAdapter extends CursorRecyclerViewAdapter<QuoteCursorAdapter.ViewHolder>
    ...
    public void onBindViewHolder(final ViewHolder viewHolder, final Cursor cursor) {
        ...
        if (Utils.showPercent) {
            String changePercentage = cursor.getString(cursor.getColumnIndex(QuoteColumns.PERCENT_CHANGE));
            viewHolder.change.setText(changePercentage);
            if(isup==1){
                viewHolder.change.setContentDescription(mContext.getString(R.string.a11y_increase_percent,changePercentage));
            } else {
                viewHolder.change.setContentDescription(mContext.getString(R.string.a11y_decrease_percent,changePercentage));
            }
        } else {
            String change = cursor.getString(cursor.getColumnIndex(QuoteColumns.CHANGE));
            viewHolder.change.setText(change);
            if(isup==1){
                viewHolder.change.setContentDescription(mContext.getString(R.string.a11y_increase_unit,change));
            } else {
                viewHolder.change.setContentDescription(mContext.getString(R.string.a11y_decrease_unit,change));
            }
        }
    }
